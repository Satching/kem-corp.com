<?php
/**
 * Template part for layout in taxonomy-request-cat.php
 *
 * @since 1.0.0
...
$breadcrumb_position = get_theme_mod( 'archive_request_breadcrumb_position', 'content_top' );
$col                 = get_theme_mod( 'archive_page_layout','two-r-col' );
$share_button_sticky = get_theme_mod( 'display_share_button_sticky_request_archive' );
?>

<div id="contents">
	<?php
		if ( ! is_paged() && 'taxonomy_term_header_full_width' === $header_style ) {
			get_template_part( 'template-parts/parts/featured-image/taxonomy-term/taxonomy-term-header-full-width' );
		} elseif ( ! is_paged() && 'taxonomy_term_header_full_width_overlay' === $header_style ) {
			get_template_part( 'template-parts/parts/featured-image/taxonomy-term/taxonomy-term-header-full-width-overlay' );
		}
	?>
	<div class="l-content">
		<?php
		...
		<div class="l-content__inner">
			...
			<?php
				get_template_part( 'template-parts/content/taxonomy-term/content-request-term' );
			?>
			</main>
			<?php
				if ( 'one-col' !== $col ) {
